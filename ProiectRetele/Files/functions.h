...
int searchInServer(int descriptor,char fileName[30])
{
  DIR *dir;
  struct dirent *file;
  dir = opendir ("/root/Documents/ProiectRetele/ServerFiles");
  if(dir==NULL)
  {
...
void primireComenzi(int descriptor) 
{
  ...
  printf("[server]The message recived:%s",message);
  //printf("%s",firstWord);
  ...
  else if (strcmp(message,"exit\n")==0 && (typeOfUser==0 || typeOfUser==1))
  {
    ...
    if(strcmp(answer,"yes")==0) {
      printf("[server]Am inchis conexiunea cu clientul\n");
      close(descriptor);
      exit(0);
    }
...
void login (int descriptor)
{
        ...
        char newPassword[50]; 
        int bytes;
        bytes = read (descriptor, msg, sizeof (buffer));
        if (bytes < 0)
       {
       ...
       while(password[i]!='\0')
      {
       ...
       strcat(nume,spatiu);
       int x=searchInFile(nume,newPassword);
       if(x){
              bzero(msg,100);
              strcat(msg,"Connected");
              printf ("[server]Message sent:%s\n",msg);
              typeOfUser=typeUser(nume);
              //printf("%d",typeOfUser);
            }
       else
      {
       ...
       if (bytes && write (descriptor, msg, bytes) < 0)
      {
